...
export default class SalaryOld extends Component {
  constructor(props) {
    super(props);
    this.state = {
      salary: [],
  ...
  onExportExcell = () => {
    const data = map(this.state.salaryChangeUpdate, (item, index) => {
      return {
        ...
        "Ngân hàng thụ hưởng/Chi nhánh (Beneficiary Bank) (4)": item.note_ngan_hang,
        "Số tiền (Amount) (5)": item.tong_luong,
        "Chi tiết thanh toán (Payment Detail) (6)": `thanh toán lương T${moment().subtract(1, "months").format("MM")}`,
