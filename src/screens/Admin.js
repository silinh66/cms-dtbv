...
export default class Admin extends Component {
    ...
    this.state = {
      ...
      column: [
        ...
        // },
        {
          ...
          // filters: salaryIndexDropdown,
          // onFilter: (value, record) => {
          //   return record.content_length === value;
          // },
          ...
          render: (ratio_bill, item) => {
            return {
              ...
              children: <div>{item.content_length}</div>,
            };
          },
        },
        {
        ...
        },
        {
          ...
          onFilter: (value, record) => {
            ...
            return includes(record.writer_name, value);
          },
        ...
        // },
        {
          ...
          filteredValue: null,
          onFilter: (value, record) => {
            return includes(record.composer_name, value);
          },
          width: 175,
          render: (value, item) => {
            const editable = this._isEditing(item);
            return editable ? (
              <Select
                ...
                onChange={(ev) => this.onChangeSelectComposerName(ev, item)}
              >
                {map(listComposerName, (item, index) => {
                  return (
                    <Option
                      className="erpOption"
                      value={[item.text, item.value, item.nick]}
          ...
          render: (ratio_bill, item) => {
            ...
            return editable ? (
              <Input
                ...
                onChange={(ev) =>
                  this.onChangeLinkAudioNote(ev.target.value, item)
                }
              />
            ) : (
              <div>{item.audio_note}</div>
            );
          },
        },
        // {
        ...
        // },
        {
          ...
          key: "editor_name",
          sorter: (a, b) => a.editor_name.length - b.editor_name.length,
          ellipsis: {
            showTitle: false,
          },
          filters: listEditorName,
          filteredValue: null,
          onFilter: (value, record) => {
            return includes(record.editor_name, value);
          },
          width: 150,
          render: (value, item) => {
            const editable = this._isEditing(item);
            return editable ? (
              <Select
                disabled={!editable}
                style={{ width: "100%" }}
                value={[item.editor_name, item.editor_code]}
                onChange={(ev) => this.onChangeSelectEditorName(ev, item)}
              >
                {map(listEditorName, (item, index) => {
                  return (
                    <Option
                      className="erpOption"
          ...
          render: (ratio_bill, item) => {
            ...
            return editable ? (
              <Input
                ...
                disabled={!editable}
                value={item.footage}
                onChange={(ev) => this.onChangeFootage(ev.target.value, item)}
              />
            ) : (
          ...
          render: (ratio_bill, item) => {
            ...
            return editable ? (
              <Input
                style={{ width: "100%" }}
                disabled={!editable}
                value={item.video_note}
                onChange={(ev) =>
                  this.onChangeLinkVideoNote(ev.target.value, item)
                }
              />
            ) : (
              <div>{item.video_note}</div>
            );
          },
        },
        {
          ...
          render: (ratio_bill, item) => {
            ...
            return editable ? (
              <Input
                ...
                onChange={(ev) =>
                  this.onChangeLinkYoutube(ev.target.value, item)
                }
              />
            ) : (
          ...
          render: (value, item) => {
            ...
            return editable ? (
              <DatePicker
                ...
                format={dateFormat}
                value={
                  !isEmpty(item.public_date) ? moment(item.public_date) : null
                }
                placeholder={"Chọn ngày public"}
                onChange={(e) => {
                  this.onChangeDatePublic(e, item);
                }}
                disabledDate={(date) => {
                  return date > moment();
                }}
  ...
  onChangeDatePublic = (value, record) => {
    const newData = map(this.state.dataChangeUpdate, (item) => {
      if (item.id === record.id) {
        return {
          ...item,
  ...
  onChangeLinkYoutube = (value, record) => {
    const newData = map(this.state.dataChangeUpdate, (item) => {
      if (item.id === record.id) {
        return {
          ...item,
  ...
  onChangeLinkAudioNote = (value, record) => {
    const newData = map(this.state.dataChangeUpdate, (item) => {
      if (item.id === record.id) {
        return {
          ...item,
  ...
  onChangeLinkVideoNote = (value, record) => {
    const newData = map(this.state.dataChangeUpdate, (item) => {
      if (item.id === record.id) {
        return {
          ...item,
  ...
  onChangeSelectComposerName = (ev, record) => {
    const newData = map(this.state.dataChangeUpdate, (item) => {
      if (item.id === record.id) {
        return {
          ...item,
  ...
  onChangeSelectEditorName = (ev, record) => {
    const newData = map(this.state.dataChangeUpdate, (item) => {
      if (item.id === record.id) {
        return {
          ...item,
  ...
  onSubmitUpdate = async (record) => {
    const response = !isEmpty(record.link_youtube)
      ? await getPublishDate(get(record, "link_youtube"))
      : null;

    const publishDate = get(response, "data.items[0].snippet.publishedAt");
    ...
    const body = {
      ...
      data: [
        ...
        record.is_first_video === 0
          ...
          : 1,
        record.composer_name !==
        this.state.dataSource[record.stt - 1].composer_name
          ? moment().format("YYYY-MM-DD HH:mm:ss")
          : record.add_composer_date === null
          ? null
          : moment(record.add_composer_date).format("YYYY-MM-DD HH:mm:ss"),
        record.editor_name !== this.state.dataSource[record.stt - 1].editor_name
          ? moment().format("YYYY-MM-DD HH:mm:ss")
        ...
        record.audio_status !==
        this.state.dataSource[record.stt - 1].audio_status
          ? moment().format("YYYY-MM-DD HH:mm:ss")
    ...
    axios.put(`${apiUrl}/tenticker`, body).then((res) => {
      ...
      //activity update composer_name
      if (
        record.composer_name !==
        this.state.dataSource[record.stt - 1].composer_name
      ) {
        const activity = `Thay đổi Composer Name của chủ đề ${
          this.state.dataSource[record.stt - 1].full_title
        } từ ${this.state.dataSource[record.stt - 1].composer_name} thành ${
          record.composer_name
        }`;
        this.onUpdateActivity(activity);
      }
      ...
      //activity update audio_status
      if (
        record.audio_status !==
        this.state.dataSource[record.stt - 1].audio_status
      ) {
        const activity = `Thay đổi Audio Status của chủ đề ${
          this.state.dataSource[record.stt - 1].full_title
        } từ ${
          this.state.dataSource[record.stt - 1].audio_status === 0
            ? "Chưa duyệt"
            : "Đã duyệt"
        } thành ${record.audio_status === 0 ? "Chưa duyệt" : "Đã duyệt"}`;
        this.onUpdateActivity(activity);
      }
      //activity update audio_note
      if (
      ...
      //activity update editor_name
      if (
        record.editor_name !== this.state.dataSource[record.stt - 1].editor_name
      ) {
        const activity = `Thay đổi Editor Name của chủ đề ${
          this.state.dataSource[record.stt - 1].full_title
        } từ ${this.state.dataSource[record.stt - 1].editor_name} thành ${
          record.editor_name
        }`;
        this.onUpdateActivity(activity);
      }
      ...
      //activity update video_status
      if (
        record.video_status !==
        this.state.dataSource[record.stt - 1].video_status
      ) {
        const activity = `Thay đổi Video Status của chủ đề ${
          this.state.dataSource[record.stt - 1].full_title
        } từ ${
          this.state.dataSource[record.stt - 1].video_status === 0
            ? "Chưa duyệt"
            : "Đã duyệt"
        } thành ${record.video_status === 0 ? "Chưa duyệt" : "Đã duyệt"}`;
        this.onUpdateActivity(activity);
      }
      //activity update video_note
      if (
      ...
      //activity update link_youtube
      if (
        record.link_youtube !==
        this.state.dataSource[record.stt - 1].link_youtube
      ) {
        const activity = `Thay đổi Link Youtube của chủ đề ${
          this.state.dataSource[record.stt - 1].full_title
        } từ ${this.state.dataSource[record.stt - 1].link_youtube} thành ${
          record.link_youtube
        }`;
        this.onUpdateActivity(activity);
      }
      //activity update public_date
      if (
        record.public_date !== this.state.dataSource[record.stt - 1].public_date
      ) {
        const activity = `Thay đổi Public Date của chủ đề ${
          this.state.dataSource[record.stt - 1].full_title
        } từ ${this.state.dataSource[record.stt - 1].public_date} thành ${
          record.public_date
        }`;
        this.onUpdateActivity(activity);
      }
  ...
  _onCloseModal = () => {
    this.setState({ isShowModal: false, chuDeInput: "", content_code: null });
  };
    ...
    const filterData = filter(this.state.newDataTest, (item) => {
      return (
        ...
        (filters.audio_status
          ...
          : item) &&
        (filters.composer_name
          ? includes(filters.composer_name, item.composer_name)
          : item) &&
        (filters.content_status
          ? includes(filters.content_status, item.content_status)
          : item) &&
        (filters.editor_name
          ? includes(filters.editor_name, item.editor_name)
          : item) &&
        (filters.full_title
  ...
  onExcel = () => {
    confirm({
      title: "Thông báo",
      content: `Bạn có chắc muốn xuất file Excel`,
      okCancel: () => {},
      onOk: () => {
        this.onExportExcell();
        // this.setState({ isLoading: true });
        // axios
        //   .delete(`https://sheetdb.io/api/v1/cajqledq2yvly/all?sheet=admin`)
        //   .then((res) => {
        //     axios
        //       .post(
        //         `https://sheetdb.io/api/v1/cajqledq2yvly?sheet=admin`,
        ...
        //           style: { backgroundColor: "#f5fff8" },
        //         });
        //         const activity = `Xuất file Excel`;
        //         this.onUpdateActivity(activity);
  ...
  onExportExcell = () => {
    const data = map(this.state.dataChangeUpdate, (item, index) => {
      return {
        STT: item.id,
        "Content Code": item.content_code,
        "Length Script": item.salary_index,
        "Full Title": item.full_title,
        "Content Raw": item.content_raw,
    ...
    const fileName = `DATA_TENTICKER${moment().format("YYYYMMDD")}`;
      ...
      switch (index) {
        ...
        case 14:
          return {
            ...col,
            filteredValue: !isEmpty(filteredInfo)
              ? filteredInfo.editor_name
              : null,
            sortOrder: !isEmpty(sortedInfo)
              ? sortedInfo.columnKey === "editor_name" && sortedInfo.order
              : null,
            onHeaderCell: (column) => ({
              width: column.width,
              onResize: this.handleResize(index),
