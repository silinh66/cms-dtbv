...
export default class CW extends Component {
  constructor(props) {
    super(props);
    const { data, listContentCode, id } = props;
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (i == id) {
        writerNick = data[i + 1][fixUserColumn.nickname];
        console.log("writerNick", data[i + 1][fixUserColumn.nickname]);
        // console.log('id', id);
    ...
    this.state = {
      ...
      column: [
        ...
        },
        {
          ...
          width: 100,
          render: (ratio_bill, item) => {
            return <div>{item.content_note}</div>;
          },
        },
  ...
  getColumnFilterTimeProps = (dataIndex) => ({
    ...
    filteredValue: null,
    onFilter: (value, record) => {
      // console.log("dateStart", value[0]);
      // console.log("dateEnd", value[1]);
      // console.log("record.public_date", record.public_date);
      // console.log(
      //   "is > dateStart?",
      //   moment(record.public_date) > moment(value[0])
      // );
      return !!record.public_date
        ? (moment(record.public_date) < moment(value[1]) &&
            moment(record.public_date) > moment(value[0])) ||
  ...
  _onRefreshData = () => {
    ...
    axios.get(`${apiUrl}/tenticker`).then((res) => {
      ...
      const dataSource = res.data.data.filter(
        (item) =>
          item.writer_name === data[id + 1][fixUserColumn.cms] &&
          item.is_new === 1
      );
      ...
      this.setState({
        ...
        isLoading: false,
        editingKey: "",
        newId: !isEmpty(res.data.data[res.data.data.length - 1])
  ...
  addTitle() {
    ...
    const body = {
      data: [
        ...
        0,
        "",
        this.state.writerNick, //writer_nick
        "",
        "",
        null,
        "",
  ...
  handleChange = (pagination, filters, sorter) => {
    ...
    const filterData = filter(this.state.newDataTest, (item) => {
      // console.log("item.public_date", item.public_date);
      return (
